#coding=utf-8
# Code to connect camera D435i and then get aligned RGB image with depth image in folder 'realsense'. 
# Lastly, get depth scale data and camera intrinsic information and write them in txt file.
# to run this code directly.
# lUluli
...
def Align_version(frames,align,show_pic=0):
    ...
    depth_colormap_aligned = cv2.applyColorMap(cv2.convertScaleAbs(depth_image_aligned, alpha=0.5), cv2.COLORMAP_JET)
...
if __name__ == "__main__":
    ...
    if USE_ROS_BAG:
        ...
        config.enable_stream(rs.stream.color, 640, 360, rs.format.bgr8, 30)  # 640, 480 ok,10、15或者30可选,20或者25会报错，其他帧率未尝试
    ...
    c_intr = profile.get_stream(rs.stream.depth)
    intrinsic = c_intr.as_video_stream_profile().get_intrinsics()
    # print('Camera intrinsic:', intrinsic)
    # print(intrinsic.ppx)
    # print(intrinsic.fx)
    with open("/home/lilulu/work/realsense/priority_test6/camera_intrinsic.txt", 'w') as f:
        f.write(str(depth_scale) +'\n' +str(intrinsic.fx)+'\n'+str(intrinsic.fy)+'\n'+str(intrinsic.ppx)+'\n'+str(intrinsic.ppy))
    
    index = 145 # start number
    try:
        while True:
            index += 1
            time.sleep(2)
            ...
            if index % 1 == 0:
                color = 'color_' + str(index) + '.png'
                depth = 'depth_' + str(index) + '.png'
                cv2.imwrite('/home/lilulu/work/realsense/priority_test6/'+ color, color_image_aligned)
                cv2.imwrite('/home/lilulu/work/realsense/priority_test6/'+ depth, depth_image_aligned)
            time.sleep(1)
            if index % 150 == 0: # end number
